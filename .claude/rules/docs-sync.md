...
Bu dokümanlar kod tabanını **elle, satır numarası ve sabit değer
referanslarıyla** anlatıyor (ör. "`data.js:369-377`", "`PENALTY_TIME = 1e7`",
"`app.js (~2550 satır)`"). Kod değiştiğinde bu referanslar sessizce
...
- **Algoritma davranışı** (`optimizer.js`/`fleet.js`): ceza katsayıları,
  ...
  kuralları → docs/TEKNIK-DOKUMAN.md §7/§8 ve README.md "Algoritma" bölümü.
- **Satır numarası/adet referansları** (`data.js:NNN-NNN` gibi alıntılar,
  dosya satır sayıları) → doğrudan grep'le doğrula, tahmin etme.
  `wc -l public/js/*.js server/*.js` ile mevcut satır sayılarını karşılaştır.
...
- **Dış servis/entegrasyon değişikliği** (yeni bir `js/*.js` istemcisi,
  yeni bir endpoint, CORS/otomasyon deseni) → docs/TEKNIK-DOKUMAN.md §10 ve
  "Bilinen sınırlar"/§13 tablosu.
- **Veri şeması** (`state` alanları, `localStorage` anahtarı, backend
  eklenirse API sözleşmesi) → docs/TEKNIK-DOKUMAN.md §5 ve §14 yol haritası.
